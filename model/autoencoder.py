...
class DecoderFC(nn.Module):
    def __init__(
        self, n_features=(256, 256), latent_dim=128, output_pts=2048, bn=False
    ):
...
class PointAE(nn.Module):
    def __init__(self, config):
        ...
        self.encoder = EncoderPointNet(
            config.enc_filters, config.latent_dim, config.enc_bn
        )
        self.decoder = DecoderFC(
            config.dec_features, config.latent_dim, config.n_pts, config.dec_bn
        )
